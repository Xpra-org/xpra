...
# ABOUTME: Exercises four NV12 bugs: .a swizzle, uniform binding, UV coords, texture allocation.

"""
Headless EGL test for the NV12 shader.

Tests:
  1. .a swizzle — V channel read from GL_LUMINANCE_ALPHA
  ...
  4. Texture allocation — UV texture width must match upload width for LUMINANCE_ALPHA
...
def render_nv12(program, y_data, uv_data, y_w, y_h, uv_w, uv_h, out_w, out_h,
    ...
    from OpenGL.GL import (
        ...
        GL_LUMINANCE, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE,
        GL_RGBA, GL_RGBA8,
    ...
    glTexImage2D(target, 0, GL_LUMINANCE, y_w, y_h, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, y_data)
    ...
    glTexParameteri(target, GL_TEXTURE_MAG_FILTER, GL_NEAREST)
    glTexParameteri(target, GL_TEXTURE_MIN_FILTER, GL_NEAREST)
    glTexImage2D(target, 0, GL_LUMINANCE_ALPHA, uv_w, uv_h, 0,
...
def main():
    ...
    # Compile the NV12 shader (with our .a fix + UV coordinate scaling)
    ...
    # With .a swizzle and V=0x40: BT.601 full-range → R≈39
    assert r < 100, f"FAIL: R={r}, expected <100 (V channel not read correctly via .a)"
    print(f"  PASS: R={r} < 100 — .a swizzle works")

    # ── Test 2: Simulate broken uniform binding ──
    # Recompile a fresh program so no uniform state carries over from test 1.
    # Then bind BOTH samplers to texture unit 0 (Y) — what happens when
    # glGetUniformLocation returns -1 and uniforms keep their default value of 0.
    fragment2 = compile_shader(nv12_source, GL_FRAGMENT_SHADER)
    program2 = link_program(vertex, fragment2)
    # Use the actual broken names from render_planar_update:
    # shader[0:1]="N", shader[1:2]="V" — neither matches the shader's "Y"/"UV" uniforms.
    ...
    # UV sampler reads GL_LUMINANCE Y texture: .r = 0.502, .a = 1.0 (luminance alpha = 1.0)
    # v = 1.0 - 0.5 = 0.5 → R = Y + 1.402*0.5 ≈ 1.2 → clamped to 255
    assert r2 > 200, f"FAIL: R={r2}, expected >200 when UV reads Y data (v=1.0→0.5)"
    print(f"  PASS: R={r2} > 200 — 'N'/'V' don't match 'Y'/'UV', UV sampler reads Y data")
    ...
    # The right edge should read garbage from uninitialized texels.
    from ctypes import c_float, c_void_p
    from OpenGL.GL import (
    ...
    glPixelStorei(GL_UNPACK_ROW_LENGTH, 0)
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1)
    glTexImage2D(target, 0, GL_LUMINANCE, y_w4, y_h4, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, y_data4)
    ...
    glTexImage2D(target, 0, GL_LUMINANCE_ALPHA, uv_w4 * 2, uv_h4, 0,
                 GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, None)
    # Upload only the correct width
    glTexSubImage2D(target, 0, 0, 0, uv_w4, uv_h4,
                    GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, uv_data4)
    ...
    tex_uv_good = glGenTextures(1)
    glActiveTexture(GL_TEXTURE1)
    glBindTexture(target, tex_uv_good)
    glTexParameteri(target, GL_TEXTURE_MAG_FILTER, GL_LINEAR)
    glTexParameteri(target, GL_TEXTURE_MIN_FILTER, GL_NEAREST)
    glTexImage2D(target, 0, GL_LUMINANCE_ALPHA, uv_w4, uv_h4, 0,
                 GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, uv_data4)

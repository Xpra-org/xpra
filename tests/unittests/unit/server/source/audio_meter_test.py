...
class TestAudioServerMeter(unittest.TestCase):
    ...
    def test_no_meter_info_without_properties(self):
        self.server.properties = typedict()
        info = self.server.get_info(None)["audio"]
        # the configuration is always exposed, so that `xpra info` can tell
        # "audio is not initialized" apart from "audio is not supported":
        assert info["initialized"] is False
        assert info["speaker"] == {"supported": True, "codecs": ()}
        # but the meter cannot run without properties:
    ...
    def test_start_and_level_info(self):
        ...
        assert info["meter"]["command"] == FakeMeter.command

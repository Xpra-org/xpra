...
def build_package() -> int:
    ...
    if is_Arch():
        ...
        with open(os.path.join(arch_dir, "PKGBUILD"), "w", encoding="utf-8") as f:
            f.write("""# Generated by `python3 ./setup.py package`
pkgname=xpra
pkgver={pkgver}
pkgrel=1
pkgdesc='Multi-platform remote display server and client'
arch=('{pkg_arch}')
url='https://xpra.org/'
license=('GPL2')

depends=('python')
makedepends=('gcc' 'make' 'binutils' 'patch' 'fakeroot' 'pkgconf' 'cython' 'python-setuptools')
...
""".format(pkgver=XPRA_VERSION, tar_name=tar_name, pkg_arch=pkg_arch))
...
def install_dev_env_command() -> list[str]:
    ...
    if is_Arch():
        flag_to_pkgs = {
            "modules": (
                "tar", "grep", "gawk", "gcc", "make", "binutils", "patch", "fakeroot", "pkgconf",
                "which", "coreutils",
                "python-cryptography",
            ),
...
toggle_packages(x11_ENABLED and clipboard_ENABLED, "xpra.x11.selection")

...
for m in pkg_resources gi cffi zeroconf; do
...
GI_MODULES="Gst GObject GLib GLibUnix GModule Gtk Gdk GtkosxApplication HarfBuzz GL Gio Pango freetype2 cairo Atk"
...
if [ "$STRIP_GSTREAMER_PLUGINS" == "1" ]; then
	...
	mkdir ${KEEP}
	PLUGINS="app audio coreelements cutter faac faad flac oss osxaudio rtp speex volume vorbis wav lame opus ogg gdp isomp4 matroska"
        if [ "${GSTREAMER_VIDEO}" == "1" ]; then
		#video support:

...
- major bugs:
   ...
   socket leak when failing to wrap as ssl
   potential race conditions with `xpra info` requests
   missing notifications due to icon parsing errors
   workaround broken svg data format generated by inkscape
   more correct solution for unicode clipboard data (MS Windows clients)
   video capability workaround for legacy packet encoders (ie: older clients)
   MacOS DMG missing code signature
- screen updates:
   fix jpeg with alpha painting with opengl backend
   png screen updates refreshed unnecessarily
   don't use an rgb format with an empty alpha channel
   ensure video screen updates are only flushed when complete
   avoid errors handling grayscale png images with transparency
   handle grayscale with spng encoder
   missing transparency with webp encoder
   paint errors in non-opengl backend
   never delay screen updates unless we have a video region
   invalid image metadata with `jpeg` decoder and `csc_cython` output
- cosmetic:
   during shutdown, ignore errors trying to set nodelay / cork
   codec loader output was ignoring standard log output format
   adjust docstring and debug logging messages, typos etc
   show disabled menu entries in MacOS menus
   log current keyboard layout whenever mapping changes
   avoid warnings triggered by Pillow v9
   error messages shown when the systray menu is not yet ready
...
- minor:
   ...
   CUDA builds with GCC 12, silence unavoidable warnings
   small X11 memory leak
   make it possible to increase the ping timeout delay
   avoid strict warning when sorting windows (`xpra list-windows`)

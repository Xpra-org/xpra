...
%changelog
* Sat Jul 22 2023 Antoine Martin <antoine@xpra.org> 4.4.7-10
- TODO
...
- major fixes:
   ...
   OpenGL failures due to misplaced icons
- network:
   generic `version` requests
   remote start error on encodings option
   remote start error when display is not specified
   silence OpenSSL3 + pycryptography legacy warning
   distinguish network vs session connected state
   parsing of ssh key files environment variable
- misc, compatibility:
...
- encodings:
   restore ability to switch encoding at runtime
   ...
   differentiate `nvjpeg` from plain `jpeg` encoder
- cosmetic:
   log buggy dpi values

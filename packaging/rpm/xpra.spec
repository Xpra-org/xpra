...
- Major:
   ...
   focus delay issuesg
   errors with `ssl` disabledg
   crashes when randr is not availableg
   failure to run on file notification setup errorsg
   enumerate all the keyboard layout groupsg
   never use a zero value as vertical refresh rateg
   DPI notification message can cause connection to drop, prevent similar issues
   webp encoder errors when scalingg
   `isX11` check would leave an unused connection openg
...
- Minor:
   ...
   skip workspace polling when unused on MS Windowsg
   preserve URI form when parsing some IP protocolsg
   preserve URI pathg
   gui dialogs go blank after being closed onceg
   rendundant events in `xpra configure features`g
   preserve window positions when re-initializing OpenGLg
   remove event receivers when windows are closedg

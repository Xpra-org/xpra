...
Caveats:
* it is also possible to run in `AES-128` or `AES-192` mode using the `XPRA_CRYPTO_KEYSIZE` environment variable, but this is not recommended
* `GCM` mode was supported from version 4.3 but had to be [disabled in version 6.2](https://github.com/Xpra-org/xpra/issues/4372)
...
The client can specify the exact mode using: `encryption=AES-CTR`.

...
class SeamlessWindowServer(WindowServer):
    ...
    def _lost_window(self, window, wm_exiting=False) -> None:
        wid = self.get_wid(window)
        # Clear our bookkeeping before notifying clients that the window is
    ...
    def _x11_focus_in_event(self, wm, event: X11Event) -> None:
        ...
        Focus packets are generated by the client, so do not synthesize an
        unfocus packet here: that could race with the client's window removal
        and focus transition.
        """
        if event.window == get_root_xid() and event.detail in (NotifyPointerRoot, NotifyDetailNone):
            focuslog("root focus (%s), clearing server focus %#x", event.detail, self._has_focus)
            self.reset_focus()

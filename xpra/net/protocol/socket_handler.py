...
class SocketProtocol:
    ...
    def start(self) -> None:
        def start_network_read_thread():
            eventlog("start_network_read_thread() closed=%s", self._closed)
            if not self._closed:
    ...
    def _invalid_header(self, proto, data:ByteString, msg="invalid packet header") -> None:
        eventlog("invalid_header(%s, %s bytes: '%s', %s)",
        ...
        while not self._closed:
            ...
            while read_buffers:
                ...
                try:
                    ...
                    eventlog(f" packet index={packet_index}, packet size={payload_size}, buffer size={bl}")
                ...
                self.receive_pending = not bool(protocol_flags & FLAGS_FLUSH)
    ...
    def do_flush_then_close(self, encoder:Optional[Callable]=None,
        ...
        def wait_for_queue(timeout:int=10) -> None:
            ...
            if not last_packet:
                close_and_release()
                return
            eventlog("flush_then_close: queue is now empty, sending the last packet and closing")
            ...
            def packet_queued(*_args):
                ...
                eventlog("flush_then_close: packet_queued() closed=%s", self._closed)

...
class DisplayClient(StubClientMixin):
    ...
    def get_dpi_caps(self) -> dict[str, Any]:
        ...
        if self.dpi > 0:
            ...
            xdpi = self.client.get_xdpi()
            ydpi = self.client.get_ydpi()
    ...
    def get_raw_vrefresh(self) -> int:
        # env override takes precedence, otherwise use the platform value;
        # (this env+clamp logic used to live on the gtk3 client's override):
    ...
    def adjust_display(self) -> None:
        if not self.server_is_monitor:
            self.verify_display_size()
        ...
        if not fequ(self.xscale, 1.0) or not fequ(self.yscale, 1.0):
            ...
            if self.server_is_desktop and not self.desktop_fullscreen:
                ...
                if self.desktop_scaling == "auto":
                    log.info(" not scaling a desktop server")
    ...
    def send_icc_data(self) -> None:
        if SYNC_ICC and (not BACKWARDS_COMPATIBLE or "configure-display" in self.get_server_packet_types()):
    ...
    def update_screen_size(self) -> None:
        ...
        log_screen_sizes(root_w, root_h, sss)
        if "configure-display" in self.get_server_packet_types() or not BACKWARDS_COMPATIBLE:

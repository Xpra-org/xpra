...
def get_global_data(data_handle) -> bytes:
...
def free_clipboard_handle(fmt: int, handle: int) -> None:
    ...
    def get_clipboard_uris(self, callback: Callable[[list[str]], None], errback: Callable[[str], None]):
        def get_uris() -> bool:
            ...
            if not data_handle:
                ...
                return False
            hdrop = get_global_data(data_handle)
            if not hdrop:
                log("failed to read CF_HDROP handle %#x (may try again)", data_handle)
                return False
            try:
    ...
    def get_clipboard_html(self, callback: Callable[[bytes], None], errback: Callable[[str], None]):
        def get_html() -> bool:
            ...
            if not data_handle:
                ...
                return False
            cf_html = get_global_data(data_handle)
            if not cf_html:
                log("failed to read HTML Format handle %#x (may try again)", data_handle)
                return False
            try:
        ...
        if COMPRESSED_IMAGES:
            ...
            if fmt:
                try:
                    ...
                    log.estr(e)
        try:
            if bitmap := data_to_bitmap(img_data):
                image_formats[win32con.CF_BITMAP] = bitmap
        except Exception as e:
            log("data_to_bitmap(%s)", Ellipsizer(img_data), exc_info=True)

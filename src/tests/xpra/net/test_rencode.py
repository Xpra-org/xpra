...
def test_hello():
    ...
    for k in data:
        if k not in l:
            print("key %s missing from reloaded dict!" % k)
            continue
        v = data.get(k)
        lv = l.get(k)
        if v!=lv:
            print("value for key %s differs, expected %s but got %s" % (k, v, lv))

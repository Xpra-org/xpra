...
class ContentPane (gtk.Notebook):
    ...
    def _construct_tab_view (self, web_view, url=None):
        ...
        scrolled_window.get_hscrollbar().connect("change-value", hscroll)
        scrolled_window.get_vscrollbar().connect("change-value", vscroll)
...
class WebBrowser(gtk.Window):
    ...
    def __init__(self):
        ...
        #added so we can hook into the browser signals from outside..
...
vscroll_listeners = []
hscroll_listeners = []
def call_scroll_cb(listeners, scrollbar, scrolltype, value):
    #print("call_scroll_cb%s" % str((listeners, scrollbar, scrolltype, value)))
    for x in listeners:
        x(scrollbar, scrolltype, value)
def vscroll(scrollbar, scrolltype, value):
    global vscroll_listeners
    call_scroll_cb(vscroll_listeners, scrollbar, scrolltype, value)

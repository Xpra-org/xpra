...
class WindowPerformanceStatistics(object):
    ...
    def get_client_backlog(self):
        ...
        if len(self.damage_ack_pending)>0:
            ...
            drop_missing_acks = []
            for sequence, (start_send_at, _, start_bytes, end_send_at, end_bytes, pixels) in tuple(self.damage_ack_pending.items()):
    ...
    def get_packets_backlog(self):
        ...
        if len(self.damage_ack_pending)>0:
            sent_before = time.time()-(self.target_latency+0.020)
            for _, (start_send_at, _, _, end_send_at, _, _) in tuple(self.damage_ack_pending.items()):

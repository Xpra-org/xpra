...
class XpraServer(gobject.GObject, X11ServerBase):
    ...
    def __init__(self, clobber):
        ...
        self.configure_damage_timer = None
        self._tray = None
    ...
    def _process_configure_window(self, proto, packet):
        ...
        if damage and not self.configure_damage_timer:
            self.schedule_configure_damage(window)

    def schedule_configure_damage(self, window):
        def damage():
            self.configure_damage_timer = None
            if window.is_managed():
                w, h = window.get_dimensions()
                self._damage(window, 0, 0, w, h)
        self.configure_damage_timer = self.timeout_add(CONFIGURE_DAMAGE_RATE, damage)

    def cancel_configure_damage(self):
        cdt = self.configure_damage_timer
        if cdt:
            self.configure_damage_timer = None
            self.source_remove(cdt)
